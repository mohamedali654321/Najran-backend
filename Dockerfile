#Build base image
FROM ubuntu:latest as dspace_base
...
#Set Pre Dspace Info ENVs ,so you can pass dspace info at a run time(when to run a container):
    ## DS ENVs:
ENV DS_HOST=http://10.200.19.15:5550
ENV DS_NAME="DSpace 7 demo from Najran University(Dev-server)"
ENV DS_UI_HOST=http://10.200.19.15:5552
ENV DS_SOLR_HOST=10.200.19.15
ENV DS_SOLR_PORT=5551
    ##DB ENVs:
ENV DB_PRE_CONFIG=false
ENV DB_HOST=10.200.19.15
ENV DB_ADMIN_USER=postgres
ENV DB_ADMIN_PASS=Kwaretech2022#
ENV DB_NAME=dspace7_4_db
ENV DB_USER=dspace7_4_db
ENV DB_PASS=dspace7_4_db
...
ENV iiif_server='http://10.200.19.15:8182/iiif/3/'
